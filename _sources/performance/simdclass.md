...
class SIMD<double,4> {
  ...
  public:
    ...
    void store (double *p) { _mm256_storeu_pd(p, m_val); }
};
...
You find examples in the file *demos/demo_simd.cpp*.
...
For the $n \text{ mod } 4$ elements at the end one could start a scalar loop, or use masking:
```cpp
size_t i = 0;
for ( ; i+4 <= n; i += 4)
